  ...
  class BatchAssembler
    ...
    def assemble_locked
      ...
      scope.limit(Configuration.max_batch_topics).each do |generation|
        admitted = admit(generation, remaining_budget)
        next unless admitted

        source, cost = admitted
    ...
    def admit(generation, remaining_budget)
      if generation.prepared_at?
        source = SnapshotPreparer.call(generation)
        return unless source && generation.reload.queued?

        cost = generation.estimated_cost_microusd.to_i
        return if cost > remaining_budget

        return [ source, cost ]
      end

      estimate = Eligibility.call(generation.topic)
      ...
      return if estimate.eligible? && estimate.estimated_cost_microusd > remaining_budget

      source = SnapshotPreparer.build(generation)
      return unless source && generation.reload.queued?

      cost = Eligibility.estimated_cost(input_tokens: source.estimated_input_tokens,
                                        output_tokens: Configuration.max_output_tokens)
      return if cost > remaining_budget

      source = SnapshotPreparer.freeze!(generation, source)
      return unless source && generation.reload.queued?

      [ source, generation.estimated_cost_microusd.to_i ]
    end
