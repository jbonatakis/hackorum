class CreateTopicSummaries < ActiveRecord::Migration[8.0]
  ...
  def change
    ...
    create_table :topic_summary_requests do |t|
      t.references :topic_summary_generation, null: false, foreign_key: true
    ...
    add_index :topic_summary_requests,
              [ :topic_summary_generation_id, :user_id ],
              unique: true,
              name: "idx_topic_summary_requests_unique"
    add_index :topic_summary_requests, [ :user_id, :quota_consumed_at ]
    ...
    add_index :topic_summaries, [ :topic_id, :source_fingerprint ], unique: true

    add_index :activities,
              [ :user_id, :subject_id ],
              unique: true,
              where: "subject_type = 'TopicSummary' AND activity_type = 'ai_summary_ready'",
              name: "idx_activities_unique_ai_summary_ready"
  end
end
