...
RSpec.describe TopicSummaryGeneration, type: :model do
  ...
  it "rejects duplicate source fingerprints for one topic" do
    topic = create(:topic)
    ...
    duplicate = build(:topic_summary_generation, topic:, state: "terminal_failed", last_message: message, source_fingerprint: "same")
    expect(duplicate).not_to be_valid
    expect(duplicate.errors[:source_fingerprint]).to be_present
  end
